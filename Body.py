...
def getKeyPoints(output,frameWidth,frameHeight,frame):	
	...
	for i in range(len(BODY_PARTS)-1):
		...
		#print(prob)
		if prob > 0.3 : 
			cv.circle(frame, (int(x), int(y)), 1, (0, 255, 255), thickness=-1, lineType=cv.FILLED)
			cv.putText(frame, "{}".format(i), (int(x), int(y)), cv.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1, lineType=cv.LINE_AA)
